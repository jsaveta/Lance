...
@Override
	...
	public Model executeStatement(Statement st) {
		...
		model.add(newSubject, SesameBuilder.sesameValueFactory.createURI(owlSameAs) , sourceSubject); 
		...
		this.worker.getGS().WriteGSAsTriples(st.getSubject().stringValue(), sourceSubject.stringValue(),0.0,"SameAs",st.getObject().stringValue(),this.worker.getGSSesameModel(),this.worker.getGSFileOutputStream());
		this.worker.getGS().WriteGSAsTriples(st.getSubject().stringValue(), newSubject.stringValue(),0.0,"SameAs",st.getObject().stringValue(),this.worker.getGSSesameModel(),this.worker.getGSFileOutputStream());
		return model;
	}
}
