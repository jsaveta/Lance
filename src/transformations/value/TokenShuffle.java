 ...
 * @date 19/mag/2010
 */
public class TokenShuffle implements DataValueTransformation {
	
	private String splitter;
	private double severity;
	
	public TokenShuffle(String splitter, double severity){
			...
			if(tokens.length > 1){
				...
				for(int i = 0; i < tokens.length; i++){
					if(coin.nextDouble() <= this.severity){
						...
						int place = coin.nextInt(tokens.length);
						tokens[i] = tokens[place];
						tokens[place] = move;
					}
